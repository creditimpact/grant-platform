...
validation hints. The folder ships with a `schema.json` that validates new
entries plus a `catalog.json` describing every supported evidence type.
...
`shared/normalization/document_aliases.json`. All services (analyzer,
eligibility engine, server) normalize document and form identifiers through this
...
3. Register any legacy names in `shared/normalization/document_aliases.json` so
...
Example: to support IRS Form 941-X uploads, create a `document_library/IRS_941X.json`
entry (if user-uploaded evidence is allowed) or a `form_templates/IRS_941X.json`
template (if the system generates the filing). Add aliases such as `"941-X"` to
`shared/normalization/document_aliases.json` so upstream services resolve user
inputs to the canonical `IRS_941X` key, and reference the canonical name in the
relevant grant JSON files.
